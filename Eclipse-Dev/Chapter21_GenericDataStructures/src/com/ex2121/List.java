...
public class List<T> {
	...
	public void search(T searchItem) {
		System.out.println(searchHelper(firstNode, searchItem));
	}
	
	/**
	 * Helper method to search the list for specific value
	 * @param node the starting node, usually the head of the list
	 * @param searchItem the value to search the list for
	 * @return <b>null</b> if no items is found or the <b>value</b> if it is found in the list
	 */
	private T searchHelper(ListNode<T> node, T searchItem) {
		if (node == null)
			return null;
		...
		if (node.data.equals(searchItem)) {
			...
			return searchHelper(node.nextNode, searchItem);
		}
	}
